...
class MS_Addon_Invitation extends MS_Addon {
	...
	public function register( $list ) {
		$list[ self::ID ] = (object) array(
			'name' => __( 'Einladungscodes', 'membership2' ),
			'description' => __( 'Benutzer benötigen einen Einladungscode, um eine Mitgliedschaft zu abonnieren.', 'membership2' ),
	...
	public function menu_item( $items, $limited_mode, $controller ) {
		if ( ! $limited_mode ) {
			$menu_item = array(
				self::ID => array(
					'title' => __( 'Einladungscodes', 'membership2' ),
					'slug' => self::SLUG,
				)
			);
	...
	public function membership_option( $view, $membership ) {
		...
		$fields = array(
			...
			array(
				...
				'title' => __( 'Benötigt diese Mitgliedschaft einen Einladungscode?', 'membership2' ),
				...
				'after' => sprintf(
					...
					__( 'Nein (öffentliche Mitgliedschaft)', 'membership2' )
				),
				'class' => 'reverse',
				'ajax_data' => array(
					'action' => $action,
		...
		if ( ! empty( $data['invitation_valid'] ) ) {
			$fields = array(
				...
				'remove_invitation_code' => array(
					...
					'title' => sprintf(
						__( 'Einladungscode %s verwenden.', 'membership2' ),
						'<strong>' . $code . '</strong>'
					),
					'button_value' => 1,
				),
			...
			$fields = array(
				...
				'apply_invitation_code' => array(
					...
					'type' => MS_Helper_Html::INPUT_TYPE_SUBMIT,
					'value' => __( 'Einladung anwenden', 'membership2' ),
				),
			);

			if ( ! $message ) {
				$message = __( 'Du benötigst eine Einladung, um Dich für diese Mitgliedschaft zu registrieren', 'membership2' );
			}
		}
				...
				<form method="post">
					...
					<div class="invitation-entry">
						<?php if ( ! isset( $data['invitation_valid'] ) ) : ?>
							...
							_e( 'Hast Du einen Einladungscode?', 'membership2' );
							?></div>
						<?php endif;
