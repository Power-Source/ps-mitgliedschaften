...
class MS_Addon_Coupon extends MS_Addon {
	...
	public function init() {
		if ( self::is_active() ) {
			$hook = 'mitgliedschaften_page_' . MS_Controller_Plugin::MENU_SLUG . '-' . self::SLUG;
	...
	public function register( $list ) {
		$list[ self::ID ] = (object) array(
			'name' => __( 'Gutscheine', 'membership2' ),
			'description' => __( 'Aktiviere Rabattgutscheine.', 'membership2' ),
	...
	public function menu_item( $items, $limited_mode, $controller ) {
		if ( ! $limited_mode ) {
			$menu_item = array(
				self::ID => array(
					'title' => __( 'Gutscheine', 'membership2' ),
					'slug' => self::SLUG,
				),
			);
	...
	public function admin_coupon() {
		...
		if ( self::validate_required( $isset, 'GET', false )
			...
			$data['memberships'] = array( __( 'Jede', 'membership2' ) );
	...
	public function enqueue_styles() {
		if ( isset( $_GET['action'] ) && 'edit' == $_GET['action'] ) {
			// Legacy jQuery UI styles removed.
		}
	...
	public function enqueue_scripts() {
		if ( isset( $_GET['action'] ) && 'edit' == $_GET['action'] ) {
			...
			wp_enqueue_script(
				'ms-view-coupon-edit',
				$plugin_url . '/premium/addon/coupon/assets/js/edit.js',
				array( 'jquery' )
			);
		}
	...
	public function billing_columns( $columns, $currency ) {
		$new_columns = array(
			...
			'discount' => __( 'Rabatt', 'membership2' ),
		);
		...
		if ( ! empty( $data['coupon_valid'] ) ) {
			$fields = array(
				...
				'remove_coupon_code' => array(
					...
					'value' => __( 'Entfernen', 'membership2' ),
			...
			$fields = array(
				...
				'apply_coupon_code' => array(
					...
					'type' => MS_Helper_Html::INPUT_TYPE_SUBMIT,
					'value' => __( 'Gutschein anwenden', 'membership2' ),
				),
			);
				...
				<form method="post">
					<?php if ( $coupon_message ) : ?>
						...
						if ( ! empty( $_POST['remove_coupon_code'] ) ) {
							printf(
								__( 'Gutschein entfernt: "%s"', 'membership2' ),
								$coupon->code
							);
						} else {
							echo $coupon_message;
					...
					<div class="coupon-entry">
						<?php if ( ! isset( $data['coupon_valid'] ) ) : ?>
							...
							_e( 'Hast Du einen Gutscheincode?', 'membership2' );
							?></div>
						<?php endif; ?>
	...
	public function maybe_apply_coupon( $invoice, $subscription ) {
		...
		if ( ! empty( $coupon['id'] ) ) {
			...
			$note = sprintf(
				__( 'Gutschein "%s" anwenden %s: Rabatt %s %s!', 'membership2' ),
				$coupon['code'],
				$deleted,
				$invoice->currency,
				$coupon['discount']
	...
	public function apply_discount( $invoice, $subscription ) {
		...
		if ( isset( $_POST['apply_coupon_code'] ) ) {
			...
			if ( ! empty( $_POST['remove_coupon_code'] ) ) {
				$note = sprintf(
					__( 'Entferne Gutschein "%s"', 'membership2' ),
					$coupon->code
				);
				$invoice->add_notes( $note );
		...
		if ( $coupon && $coupon->is_valid( $membership->id ) ) {
			...
			$note = sprintf(
				__( 'Gutschein "%s" anwenden: Rabatt %s %s!', 'membership2' ),
				$coupon->code,
				$invoice->currency,
				$discount
			);
	...
	public function payment_description_recurring( $desc, $short, $currency, $total_price, $membership, $invoice ){
		...
		if ( isset( $_POST['apply_coupon_code'] ) && ! empty( $_POST['coupon_code'] ) && ! empty( $_REQUEST['membership_id'] ) ) {
			...
			if ( $membership->pay_cycle_repetitions > 1 ) {
				...
				if ( $short ) {
					$lbl = _n( '<span class="price">%1$s %2$s</span> einmalig und dann <span class="price">%1$s %3$s</span> (alle %4$s)', $no_payment_repeat, 'membership2' );
				} else {
					$lbl = _n( 'Erste Zahlung <span class="price">%1$s %2$s</span> und anschließend eine Zahlung von <span class="price">%1$s %3$s</span>.', 'Erste Zahlung <span class="price">%1$s %2$s</span> und anschließend %5$s Zahlungen von <span class="price">%1$s %3$s</span>, eine alle %4$s.', $no_payment_repeat, 'membership2' );
				}
			} else {
				// Indefinite number of payments
				if ( $short ) {
					$lbl = __( '<span class="price">%1$s %2$s</span> einmalig <span class="price">%1$s %3$s</span> (alle %4$s)', 'membership2' );
				} else {
					$lbl = __( 'Du bezahlst <span class="price">%1$s %2$s</span> einmalig und dann <span class="price">%1$s %3$s</span> alle %4$s.', 'membership2' );
				}
			}
