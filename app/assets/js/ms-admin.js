...
jQuery.fn.ms_datepicker = function(args) {
    var fn = window.ms_functions,
        ...
        toIso = function(val) {
            ...
            matches = val.match(/(\d{4})[-/.](\d{2})[-/.](\d{2})/);
            return matches ? [matches[1], matches[2], matches[3]].join('-') : val;
        };
    ...
    return this.each(function() {
        ...
        el.addClass(config.custom_class);

        if (supportsDate) {
            el.attr('type', 'date');
            if (current) {
                el.val(toIso(current));
            }
        } else {
            el.attr('type', 'text')
        ...
        if (config.min) { el.attr('min', config.min); }
        if (config.max) { el.attr('max', config.max); }
    });
};
