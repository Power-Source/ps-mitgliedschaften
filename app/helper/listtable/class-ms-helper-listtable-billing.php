...
class MS_Helper_ListTable_Billing extends MS_Helper_ListTable {
	...
	public function get_columns() {
		...
		$columns = apply_filters(
			...
			array(
				...
				'user' 			=> __( 'Benutzer', 'membership2' ),
				'membership' 	=> __( 'Mitgliedschaft', 'membership2' ),
				'status' 		=> __( 'Status', 'membership2' ),
				'total' 		=> __( 'Gesamt', 'membership2' ),
				'due_date' 		=> __( 'Fälligkeit', 'membership2' ),
	...
	public function column_invoice( $item ) {
		...
		if ( MS_Gateway_Manual::ID == $item->gateway_id && ! $item->is_paid() ) {
			...
			$actions['pay_it'] = sprintf(
				'<a href="%s">%s</a>',
				$action_url,
				__( 'Als bezahlt markieren', 'membership2' )
			);
		}
	...
	public function column_due_date( $item, $column_name ) {
		...
		if ( $due_now ) {
			$html 		= sprintf(
				...
				$due_date,
				__( 'Zahlung ist überfällig', 'membership2' )
			);
	...
	public function get_bulk_actions() {
		$bulk_actions = array(
			'archive' => __( 'Entfernen', 'membership2' ),
		);
	...
	public function get_views() {
		$all_status = array(
			MS_Model_Invoice::STATUS_PAID 	=> __( 'Bezahlt', 'membership2' ),
			MS_Model_Invoice::STATUS_NEW 	=> __( 'Entwurf', 'membership2' ),
			MS_Model_Invoice::STATUS_DENIED => __( 'Verweigert', 'membership2' ),
		);
		...
		$views['open'] = array(
			'url' 	=> $url,
			'label' => __( 'Abgerechnet oder ausstehend', 'membership2' ),
			'count' => $count,
		);
