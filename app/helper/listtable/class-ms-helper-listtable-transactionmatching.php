...
class MS_Helper_ListTable_TransactionMatching extends MS_Helper_ListTable {
	...
	public function get_columns() {
		...
		$columns = apply_filters(
			...
			array(
				...
				'date' 		=> __( 'Zeit', 'membership2' ),
				'status' 	=> '',
				'method' 	=> '',
				'gateway' 	=> __( 'Gateway', 'membership2' ),
				'amount' 	=> __( 'Betrag', 'membership2' ),
				'invoice' 	=> __( 'Rechnung', 'membership2' ),
	...
	protected function get_source_label( $source, $source_id ) {
		$label = __( 'Unbekannte ID: %s', 'membership2' );

		switch ( $source ) {
			case 'pay_btn':
				$label = __( 'PayPal-Zahlungsschaltfläche #%s', 'membership2' );
				break;

			case 'm1':
			default:
				$label = __( 'M1 Abonnementstufe #%s', 'membership2' );
				break;
		}
	...
	public function get_views() {
		...
		if ( MS_Model_Import::can_match() ) {
			...
			$views['label'] = array(
				'label' => __( 'Undefinierte Transaktionen:', 'membership2' ),
			);
	...
	public function views() {
		...
		if ( ! $this->matching_type || ! $this->matching_type_id ) {
			if ( ! MS_Model_Import::can_match() ) {
				...
				echo '<p>';
				_e( 'Keine passende Transaktion gefunden.', 'membership2' );
				echo '</p><p>';
				printf(
					'<strong>%s</strong><br />',
					__( 'Momentan nichts zu tun:', 'membership2' )
				);
				_e( 'Transaktionen, die automatisch abgeglichen werden können, werden hier angezeigt, wenn sie von einem Zahlungsgateway verarbeitet werden.<br>Überprüfe dies einfach später, nachdem neue Zahlungen getätigt wurden.', 'membership2' );
				echo '</p><p>';
				printf(
					__( 'Wenn Du ungeduldig bist, wiederhole einige Fehlerstatus-Transaktionen im Abschnitt %sTransaktionsprotokolle%s und prüfe, ob sie auf dieser Seite angezeigt werden.', 'membership2' ),
					'<a href="' . $url . '">',
					'</a>'
				);
				echo '</p>';
		...
		<form class="transaction-matching">
			...
			<div class="content">
				<p><?php
				printf(
					__( '1. Unten findest Du eine Liste möglicher "%s"-Transaktionen. Untersuche diese Transaktionen, um herauszufinden, auf welche Mitgliedschaft sie sich beziehen.', 'membership2' ),
					'<b>' . $label . '</b>'
				);
				?></p>
				<hr />
				...
				<?php
				_e( 'Hinweis:', 'membership2' );
				?>
				<br />
				<?php
				_e( 'Diese Auswahl wird gespeichert, sodass neue Transaktionen von nun an automatisch verarbeitet werden.', 'membership2' );
				?>
				<br />
				<?php
				_e( 'Nach dem Speichern werden alle unten aufgeführten Transaktionen verarbeitet. Dies kann eine Weile dauern.', 'membership2' );
				?>
				</div>
			</div>
			<div class="buttons">
	...
	public function column_method( $item, $column_name ) {
		...
		$icon = '';
		$info = __( 'Unbekannte Methode', 'membership2' );

		switch ( $item->method ) {
			case 'handle':
				$icon = 'fa-cloud-download';
				$info = __( 'Das Gateway hat die IPN-URL aufgerufen', 'membership2' );
				break;

			case 'request':
				$icon = 'fa-refresh';
				$info = __( 'Plugin hat eine wiederkehrende Zahlung angefordert', 'membership2' );
				break;

			case 'process':
				$icon = 'fa-shopping-cart';
				$info = __( 'Der Benutzer hat die Zahlungsdetails eingegeben', 'membership2' );
				break;
		}
