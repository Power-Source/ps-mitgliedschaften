...
class MS_Helper_ListTable_Membership extends MS_Helper_ListTable {
	...
	public function get_columns() {
		$columns = array(
			...
			'priority' 			=> sprintf(
				'<span title="%s">#</span>',
				__( 'Mitgliedschaften Sortierung', 'membership2' )
			),
			'name' 				=> __( 'Mitgliedschaftsname', 'membership2' ),
			'active' 			=> __( 'Aktiv', 'membership2' ),
			'type_description' 	=> __( 'Mitgliedschaftstyp', 'membership2' ),
			'members' 			=> __( 'Mitglieder', 'membership2' ),
			'price' 			=> __( 'Zahlung', 'membership2' ),
	...
	public function column_name( $item ) {
		...
		if ( ! $item->is_system() ) {
			$actions['payment'] = sprintf(
				...
				esc_attr( $item->id ),
				$item->is_free ? __( 'Zugriffsoptionen', 'membership2' ) : __( 'Zahlungsmöglichkeiten', 'membership2' )
			);
		}
		...
		if ( $item->is_guest() ) {
			$badge = sprintf(
				...
				__( 'Gast', 'membership2' ),
				__( 'Alle abgemeldeten Benutzer gelten als Gäste', 'membership2' )
			);
		} elseif ( $item->is_user() ) {
			$badge = sprintf(
				'<span class="ms-badge ms-user-badge" data-wpmui-tooltip="%2$s" data-width="180">%1$s</span>',
				__( 'Standard', 'membership2' ),
				__( 'Alle angemeldeten Benutzer, die sich nicht für eine Mitgliedschaft angemeldet haben', 'membership2' )
			);
		} else {
			$badge = apply_filters(
	...
	public function column_shortcode( $item, $column_name ) {
		$shortcodes = array(
			MS_Rule_Shortcode_Model::PROTECT_CONTENT_SHORTCODE => array(
				'tag' 	=> '[%1$s id="%2$s"][/%1$s]',
				'label' => __( 'Inhalte schützen', 'membership2' ),
			),
			MS_Helper_Shortcode::SCODE_MS_TITLE => array(
				'tag' 	=> '[%1$s id="%2$s"]',
				'label' => __( 'Mitgliedschaftstitel', 'membership2' ),
			),
			MS_Helper_Shortcode::SCODE_MS_DETAILS => array(
				'tag' 	=> '[%1$s id="%2$s"]',
				'label' => __( 'Beschreibung der Mitgliedschaft', 'membership2' ),
			),
			MS_Helper_Shortcode::SCODE_MS_PRICE => array(
				'tag' 	=> '[%1$s id="%2$s"]',
				'label' => __( 'Mitgliedschaftspreis', 'membership2' ),
			),
			MS_Helper_Shortcode::SCODE_MS_BUY => array(
				'tag' 	=> '[%1$s id="%2$s"]',
				'label' => __( 'Abonnement-Schaltfläche', 'membership2' ),
			),
			MS_Helper_Shortcode::SCODE_REGISTER_USER => array(
				'tag' 	=> '[%1$s membership_id="%2$s"]',
				'label' => __( 'Registrierungsformular', 'membership2' ),
			),
		);
		if ( $item->is_system() ) {
		...
		$html = sprintf(
			...
			__( 'Zeigen', 'membership2' ) . ' <i class="wpmui-fa wpmui-fa-caret-down"></i>',
			implode( '', $lines )
		);
