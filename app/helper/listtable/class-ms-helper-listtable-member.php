...
class MS_Helper_ListTable_Member extends MS_Helper_ListTable {
	...
	public function get_columns() {
		$columns = array(
			...
			'username' 		=> __( 'Benutzername', 'membership2' ),
			'email' 		=> __( 'E-Mail', 'membership2' ),
			'membership' 	=> __( 'Mitgliedschaft', 'membership2' ),
			'infos' 		=> '&nbsp;',
		);
	...
	public function column_username( $member ) {
		...
		if ( !MS_Model_Member::is_admin_user( $member->id ) ) {
			$actions['edit'] = sprintf(
				...
				__( 'Abonnementdetails', 'membership2' )
			);
		}
		$actions['profile'] = sprintf(
			'<a href="%s">%s</a>',
			admin_url( 'user-edit.php?user_id=' . $member->id ),
			__( 'Profil bearbeiten', 'membership2' )
		);
	...
	public function column_membership( $member ) {
		if ( MS_Model_Member::is_admin_user( $member->id ) ) {
			$html = '<b>' . __( 'Administrator', 'membership2' ) . '</b>';
		} else {
	...
	public function get_bulk_actions() {
		...
		$bulk_actions 	= array(
				'drop-all' 		=> __( 'Lösche alle Mitgliedschaften', 'membership2' ),
				$protect_key 	=> array(),
				$unprotect_key 	=> array(),
		);
	...
	public function searchbox_filters() {
		...
		$search_options = array(
			...
			'field_options' => array(
				'username'   => __( 'Benutzername/E-Mail', 'membership2' ),
				'nickname'   => __( 'Nickname', 'membership2' ),
				'first_name' => __( 'Vorname', 'membership2' ),
				'last_name'  => __( 'Nachname', 'membership2' ),
			),
		);
	...
	public function get_views() {
		...
		$views['label'] = array(
			'label' => __( 'Abonnementstatus:', 'membership2' ),
		);
		...
		if ( empty( $_REQUEST['membership_id'] ) ) {
			...
			$views['all'] = array(
				'url' 	=> $url,
				'label' => __( 'Alle Benutzer', 'membership2' ),
			);
		} else {
		...
		$views['active'] 	= array(
			'url' 	=> $url,
			'label' => __( 'Aktives Abonnement', 'membership2' ),
			'count' => $count,
		);
		...
		$views['cancelled'] = array(
			'url' 	=> $url,
			'label' => __( 'Abgebrochen', 'membership2' ),
			'count' => $count,
		);
		...
		$views['expired'] 	= array(
			'url' 	=> $url,
			'label' => __( 'Abgelaufen', 'membership2' ),
			'count' => $count,
		);
		...
		$views['pending'] 	= array(
			'url' 	=> $url,
			'label' => __( 'Ausstehend', 'membership2' ),
			'count' => $count,
		);
