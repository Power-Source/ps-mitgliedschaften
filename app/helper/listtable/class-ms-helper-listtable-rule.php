...
class MS_Helper_ListTable_Rule extends MS_Helper_ListTable {
	...
	public function __construct( $model ) {
		...
		$this->name['default_access'] 	= __( 'Jedermann', 'membership2' );
	...
	public function get_columns() {
		return apply_filters(
			...
			array(
				...
				'content' 	=> __( 'Inhalt', 'membership2' ),
				'rule_type' => __( 'Regeltyp', 'membership2' ),
				'dripped' 	=> __( 'Gedropfter Inhalt', 'membership2' ),
			)
		);
	}
	...
	public function get_bulk_actions() {
		$protect_key 	= __( 'Mitgliedschaft hinzufügen', 'membership2' );
		$unprotect_key 	= __( 'Mitgliedschaft verwerfen', 'membership2' );
		$bulk_actions 	= array(
			'rem-all' 		=> __( 'Lösche alle Mitgliedschaften', 'membership2' ),
			$protect_key 	=> array(),
			$unprotect_key 	=> array(),
		);

		$memberships = MS_Model_Membership::get_membership_names();
		$txt_add = __( 'Hinzufügen: %s', 'membership2' );
		$txt_rem = __( 'Verwerfen: %s', 'membership2' );
	...
	public function prepare_items() {
		...
		if ( $is_dripped ) {
			$this->_column_headers[0]['dripped'] = __( 'Inhalt anzeigen', 'membership2' );
		} else {
			unset( $this->_column_headers[0]['dripped'] );
		}

		if ( isset( $this->_column_headers[0]['access'] ) ) {
			$this->_column_headers[0]['access'] = __( 'Wer hat Zugang', 'membership2' );
		}
	...
	public function column_access( $item, $column_name ) {
		...
		$public = array(
			...
			'after' => __( 'Ändere den Zugriff' , 'membership2' ),
			'class' => 'ms-empty-note',
		);
		if ( !empty( $rule->rule_value ) && array_key_exists( $item->id, $rule->rule_value ) ) {
			$public['value'] = __( 'Keine (zugewiesene Mitgliedschaft wurde gelöscht)', 'membership2' );
		}
	...
	public function column_dripped( $item, $column_name ) {
		...
		if ( empty( $label ) ) {
			$label = __( 'Datum einstellen...', 'membership2' );
		}
	...
	protected function inline_edit() {
		...
		$field_date 	= array(
			...
			'name' 			=> 'date',
			'placeholder' 	=> __( 'Datum', 'membership2' ) . '...',
		);
		...
		$field_delay_type = array(
			...
			'after' 		=> __( 'nach dem Abonnement', 'membership2' ),
		);
		...
		<div>
			<h4>
				...
				<?php _e( 'Einstellungen für Tropfinhalte', 'membership2' ); ?>
			</h4>
		</div>
		<fieldset>
	...
	public function list_head() {
		...
		if ( empty( $_GET['membership_id'] ) ) {
			if ( empty( $_GET['status'] ) ) {
				$title = __( 'Zeige <b>Alle</b> %1$s', 'membership2' );
			} elseif ( MS_Model_Rule::FILTER_NOT_PROTECTED == $_GET['status'] ) {
				$title = __( 'Alle <b>ungeschützten</b> %1$s anzeigen', 'membership2' );
			} elseif ( MS_Model_Rule::FILTER_PROTECTED == $_GET['status'] ) {
				$title = __( 'Alle <b>geschützten</b> %1$s anzeigen', 'membership2' );
			}
		} else {
			$membership = MS_Factory::load( 'MS_Model_Membership', $_GET['membership_id'] );

			if ( empty( $_GET['status'] ) ) {
				$title = __( '<b>Alle</b> %1$s für %2$s anzeigen', 'membership2' );
			} elseif ( MS_Model_Rule::FILTER_NOT_PROTECTED == $_GET['status'] ) {
				$title = __( 'Alle %1$s anzeigen, die <b>nicht durch %2$s</b> geschützt sind', 'membership2' );
			} elseif ( MS_Model_Rule::FILTER_PROTECTED == $_GET['status'] ) {
				$title = __( 'Anzeigen aller %1$s, die <b>durch %2$s</b> geschützt sind', 'membership2' );
			}
	...
	public function get_views() {
		...
		$views['public'] = array(
			'url' 	=> $public_url,
			'label' => __( 'Ungeschützt', 'membership2' ),
		...
		$views['protected'] = array(
			'url' 	=> $protected_url,
			'label' => __( 'Geschützt', 'membership2' ),
