...
class MS_Helper_ListTable_TransactionLog extends MS_Helper_ListTable {
	...
	public function get_views() {
		...
		$views['all'] = array(
			'label' => __( 'Alle', 'membership2' ),
			'url' 	=> $base_url,
			'count' => MS_Model_Transactionlog::get_item_count(),
		);

		$views['ok'] = array(
			'label' => __( 'Erfolgreich', 'membership2' ),
		...
		$views['err'] = array(
			'label' => __( 'Gescheitert', 'membership2' ),
		...
		$views['ignore'] = array(
			'label' => __( 'Ignoriert', 'membership2' ),
	...
	public function column_note( $item, $column_name ) {
		...
		if ( 'err' == $item->state ) {
			...
			$actions['action-ignore'] 	= __( 'Ignorieren', 'membership2' );

			// We can only re-process the transaction if we have POST data.
			$postdata = $item->post;
			if ( is_array( $postdata ) && ! empty( $postdata ) ) {
				$actions['action-retry'] = __( 'Wiederholen', 'membership2' );
			}
		} elseif ( 'ignore' == $item->state ) {
			if ( $item->is_manual ) {
				$actions['action-clear'] = __( 'Zurücksetzen', 'membership2' );
	...
	static public function get_details( $item ) {
		...
		if ( $item->is_manual ) {
			$detail_lines[] = __( 'Transaktionsstatus manuell geändert', 'membership2' );
			$detail_lines[] = sprintf(
				__( 'Geändert am: %s', 'membership2' ),
				$item->manual_date
			);
			$detail_lines[] = sprintf(
				__( 'Angepasst von: %s', 'membership2' ),
		...
		if ( ! empty( $postdata ) && is_array( $postdata ) ) {
			...
			switch ( $item->gateway_id ) {
				case MS_Gateway_Paypalstandard::ID:
					if ( isset( $postdata['invoice'] ) ) {
						...
						$id_fields[] 	= 'custom';
						$detail_lines[] = __( 'Importiertes Abonnement vom alten Mitgliedschafts-Plugin.', 'membership2' );
					} elseif ( isset( $postdata['btn_id'] ) ) {
						$id_fields[] 	= 'btn_id';
						$id_fields[] 	= 'payer_email';
						$detail_lines[] = __( 'Zahlung über eine PayPal-Zahlungsschaltfläche.', 'membership2' );
					} elseif ( isset( $postdata['txn_type'] ) ) {
						// Highlight invalid transactions.
						if ( 'send_money' == $postdata['txn_type'] ) {
							$id_fields[] 	= 'txn_type';
							$detail_lines[] = __( 'Jemand hat Dir Geld innerhalb von PayPal gesendet oder PayPal hat eine vorherige Zahlung erneut gesendet.<br>Das Plugin hat nicht versucht, die Zahlung einem Abonnement zuzuordnen.', 'membership2' );
						}
					}
					break;
			}
			...
			foreach ( $postdata as $key => $value ) {
				...
				if ( 0 === $ind ) {
					$detail_lines[] = __( 'POST Daten:', 'membership2' );
				}
		...
		if ( ! empty( $headers ) && is_array( $headers ) ) {
			...
			ksort( $headers );
			$ind = 0;
			$detail_lines[] = __( 'HTTP Header:', 'membership2' );
		...
		$detail_lines[] = __( 'Angemeldeter Benutzer:', 'membership2' );
		...
		if ( $user_id ) {
			...
			$detail_lines[] = sprintf(
				...
				1,
				__( 'Gast', 'membership2' ),
				__( 'Ein angemeldeter Benutzer konnte nicht ermittelt werden', 'membership2' )
			);
		}
