...
class MS_Controller_Frontend extends MS_Controller {
	...
	public function signup_process() {
		...
		switch ( $step ) {
			...
			default:
				MS_Helper_Debug::debug_log( "Kein Handler für Schritt: $step" );
				break;
		}
	}
	...
	public function verification_notification( $content ) {
		return apply_filters(
			...
			__( 'Bitte überprüfe Deine E-Mails auf weitere Anweisungen, um Deine E-Mails zu bestätigen.', 'membership2' ),
			$content,
			$this
		);
	}
	...
	public function payment_table( $content ) {
		...
		if ( ! empty( $_POST['ms_relationship_id'] ) ) {
			...
			MS_Helper_Debug::debug_log( 'Fehler: fehlende POST-Parameter' );
		...
		switch ( $action ) {
			...
			case self::ACTION_VIEW_ACTIVATEACCOUNT:
				...
				$data_defaults 		= array(
					...
					// form="lost"
					'label_lost_username' => __( 'Benutzername oder E-Mail-Adresse', 'membership2' ),
					'label_lostpass'      => __( 'Passwort zurücksetzen', 'membership2' ),
	...
	public function enqueue_scripts() {
		...
		if ( $load_on_front_pages ) {
			$data = array(
				...
				'cancel_msg' => __( 'Bist du sicher, dass du abbrechen möchtest?', 'membership2' ),
			);
	...
	function login_message( $message ) {
		if ( isset( $_GET['ms_error'] ) ) {
			$msg = __( 'Konto nicht verifiziert. Bitte überprüfe Deine E-Mail für einen Bestätigungslink', 'membership2' );
