...
class MS_Controller_Member extends MS_Controller {
	...
	public function members_export_button() {
		...
		$csv_button = array(
			...
			'url'   => $url,
			'value' => __( 'Liste als CSV exportieren', 'membership2' ),
	...
	public function admin_page_editor() {
		...
		if ( ! empty( $_REQUEST['user_id'] ) && ( $user_id = intval( $_REQUEST['user_id'] ) ) ) {
			if ( user_can( $user_id, 'administrator' ) ) {
				wp_die( __( 'Leider darfst Du nicht auf diese Seite zugreifen.', 'membership2' ), 403 );
			}
	...
	public function ajax_action_validate_field() {
		...
		if ( $this->_resp_ok() && ! $this->is_admin_user() ) {
			...
			$this->_resp_err( __( 'Dieses Feld ist erforderlich.', 'membership2' ) );
		}
		...
		if ( $this->_resp_ok() ) {
			...
			if ( 'email' == $field ) {
				if ( ! is_email( $value ) ) {
					$msg = __( 'Ungültige E-Mail-Adresse', 'membership2' );
				} elseif ( email_exists( $value ) ) {
					$msg = __( 'E-Mail bereits vergeben', 'membership2' );
				} else {
					$msg = 1;
				}
			} elseif ( 'username' == $field ) {
				if ( username_exists( $value ) ) {
					$msg = __( 'Benutzername bereits vergeben', 'membership2' );
				} else {
					$msg = 1;
				}
			}
	...
	public function enqueue_styles() {
		// Legacy jQuery UI styles no longer needed.
	}
	...
	public function enqueue_scripts_list() {
		...
		if ( 'edit_date' == $_GET['action'] ) {
			...
			$data['lang']      = array(
				'select_user' => __( 'Wähle einen Benutzer aus', 'membership2' ),
			);
		}
	...
	public function manage_users_columns( $columns ) {
		...
		$membership_column = array( 'membership' => __( 'Mitgliedschaft', 'membership2' ) );
		if ( $settings->force_registration_verification ) {
			$membership_column = array( 'membership' => __( 'Mitgliedschaft', 'membership2' ), 'verified' => __( 'Verifiziert', 'membership2' ) );
		}
	...
	public function manage_users_custom_column( $value, $column_name, $user_id ) {
		if ( 'membership' == $column_name ) {
			...
			if ( MS_Model_Member::is_admin_user( $user_id ) ) {
				$value = '<span style="font-weight:bold;">' . __( 'Nicht (Administrator)', 'membership2' ) . '</span>';
			} else {
				$member       = MS_Factory::load( 'MS_Model_Member', $user_id );
				...
				if ( $subscription ) {
					...
					$view_url = sprintf(
						...
						$url,
						__( 'Mitglieder anzeigen', 'membership2' ),
						$membership->name
					);
			...
			if ( 'verified' == $column_name ) {
				...
				if ( $user_activation_status != 1 ) {
					if ( $force_user_activation_status ) {
						$value = __( 'Überprüfung erneut senden', 'membership2' );
					} else {
						$value = __( 'Nicht verifiziert', 'membership2' );
					}

				} else {
					$value = __( 'Verifiziert', 'membership2' );
				}
			}
		}
	...
	function add_verify_bulk_action( $actions ) {
		...
		$actions['ms_bulk_approve']    = __( 'Genehmigen', 'membership2' );
		$actions['ms_bulk_disapprove'] = __( 'Ablehnen', 'membership2' );
		$actions['ms_bulk_resend']     = __( 'Bestätigungsmail erneut senden', 'membership2' );
	...
	function handle_verify_bulk_message() {
		if ( isset ( $_REQUEST['_ms_approved'] ) ) {
            ...
            <div class="notice notice-success is-dismissible">
                <p><?php echo sprintf( __( '%d Benutzerkonten genehmigt', 'membership2' ), $user_count ); ?></p>
            </div>
			<?php
		} else {
			if ( isset ( $_REQUEST['_ms_disapproved'] ) ) {
				...
				?>
                <div class="notice notice-success is-dismissible">
                    <p><?php echo sprintf( __( '%d Benutzerkonten abgelehnt', 'membership2' ), $user_count ); ?></p>
                </div>
				<?php
			} else {
				if ( isset ( $_REQUEST['_ms_resend'] ) ) {
					...
					?>
                    <div class="notice notice-success is-dismissible">
                        <p><?php echo sprintf( __( '%d Benutzerkonten E-Mails erneut senden', 'membership2' ), $user_count ); ?></p>
                    </div>
					<?php
				}
			}
