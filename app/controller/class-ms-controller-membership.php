...
class MS_Controller_Membership extends MS_Controller {
	...
	public function admin_page_router() {
		...
		if ( self::is_valid_step( $step ) ) {
			...
			if ( method_exists( $this, $method ) ) {
				...
				MS_Helper_Debug::debug_log( "Methode $method für Schritt $step nicht gefunden" );
			}
		} else {
			MS_Helper_Debug::debug_log( "Ungültiger Schritt: $step" );
		}
	...
	public function get_edit_tabs() {
		...
		if ( null === $Tabs ) {
			...
			$Tabs 	= array(
				...
				self::TAB_TYPE => array(
					'title' => __( 'Art der Mitgliedschaft', 'membership2' ),
				),
				self::TAB_PAYMENT => array(
					'title' => __( 'Zahlungsmöglichkeiten', 'membership2' ),
				),
				self::TAB_UPGRADE => array(
					'title' => __( 'Aktualisiere die Pfade', 'membership2' ),
				),
				...
				*/
				self::TAB_MESSAGES => array(
					'title' => __( 'Schutzmeldungen', 'membership2' ),
				),
				self::TAB_EMAILS => array(
					'title' => __( 'Automatisierte E-Mail-Antworten', 'membership2' ),
				),
			);
			...
			if ( $membership->is_system() ) {
				...
				$Tabs[self::TAB_PAYMENT]['title'] = __( 'Zugriffsoptionen', 'membership2' );
			}
	...
	public function enqueue_scripts() {
		$data = array(
			'ms_init' => array(),
			'lang' => array(
				'msg_delete' 		=> __( 'Möchtest Du die Mitgliedschaft <strong>%s</strong> einschließlich aller Abonnements vollständig löschen?', 'membership2' ),
                'msg_bulk_delete' 	=> __( 'Möchtest Du alle ausgewählten Mitgliedschaften einschließlich aller Abonnements vollständig löschen?', 'membership2' ),
				'btn_delete' 		=> __( 'Löschen', 'membership2' ),
				'btn_cancel' 		=> __( 'Abbrechen', 'membership2' ),
				'quickedit_error' 	=> __( 'Fehler beim Speichern der Änderungen.', 'membership2' ),
			),
		);
