...
class MS_Controller_Plugin extends MS_Controller {
	...
	protected function check_special_view() {
		...
		if ( $view != null ) {
			...
			add_submenu_page(
				self::$base_slug,
				__( 'Mitgliedschaften', 'membership2' ),
				__( 'Mitgliedschaften', 'membership2' ),
				$this->capability,
				self::$base_slug,
	...
	public function add_menu_pages() {
		 ...
		 */
		add_menu_page(
			__( 'Mitgliedschaften', 'membership2' ), // no i18n!
			'Mitgliedschaften', // no i18n!. Translating menu could affect screen ids
			$this->capability,
			self::$base_slug,
			null,
			'dashicons-lock'
	...
	private function get_setup_menu_pages() {
		$pages = array(
			'setup' => array(
				'title' => __( 'Einrichten', 'membership2' ),
				'slug'  => '',
			),
		);
	...
	private function get_default_menu_pages() {
		...
		$pages = array(
			...
			'members'           => array(
				'title' => __( 'Alle Mitglieder', 'membership2' ),
				'slug'  => 'members',
			),
			'add-member'        => array(
				'title' => __( 'Mitglied hinzufügen', 'membership2' ),
				'slug'  => 'add-member',
			),
			'billing'           => false,
			'addon'             => array(
			...
			'settings'          => array(
				'title' => __( 'Einstellungen', 'membership2' ),
				'slug'  => 'settings',
			),
			'help'              => array(
				'title' => __( 'Hilfe', 'membership2' ),
				'slug'  => 'help',
			),
		);
		...
		if ( $show_billing ) {
			...
			if ( ! $payment_possible ) {
				mslib3()->ui->admin_message(
					sprintf(
						__( 'Hoppla, es sieht so aus, als hättest Du noch kein Zahlungsgateway aktiviert.<br />Du musst mindestens ein Gateway einrichten und aktivieren, sonst können sich Deine Mitglieder nicht für eine bezahlte Mitgliedschaft anmelden.<br />%sBehebe dies jetzt &raquo;%s', 'membership2' ),
	...
	public function add_privacy_policy_content() {
		...
		$content = __( '<h3>Zahlungsgateways</h3>
		<p class="privacy-policy-tutorial">
		    Wenn Du Zahlungen mit Stripe oder PayPal akzeptierst, werden einige Deiner Kundendaten an den jeweiligen Dritten weitergegeben.
			Einige der Daten enthalten:
			<ul>
				<li>Name</li>
				<li>Email</li>
				<li>Addresse</li>
				<li>Telefon</li>
				<li>Stadt, Staat, PLZ</li>
			</ul>
		...
		Mitglieder unseres Teams haben Zugriff auf die Informationen, die Du uns zur Verfügung stellst. Sowohl Administratoren als auch Shop-Manager können darauf zugreifen:
			<ul>
				<li>Bestellinformationen wie das, was gekauft wurde, wann es gekauft wurde und</li>
				<li>Kundeninformationen wie Dein Name, Deine E-Mail-Adresse und Rechnungsinformationen.</li>
			</ul>
		</p>',
			'membership2' );
	...
	private function admin_localize_text() {
		$messages = array(
			'recurring_cancel_warning'    => esc_html__( 'Bitte beachte, dass diese Mitgliedschaft, wenn sie ein aktives Abonnement im Zahlungsgateway hat, gekündigt wird und Du es möglicherweise später nicht wieder aktivieren kannst.', 'membership2' ),
			'membership_multiple_limit'   => MS_Model_Addon::is_enabled( MS_Model_Addon::ADDON_MULTI_MEMBERSHIPS ) ? 0 : 1,
			'membership_multiple_message' => esc_html__( 'Die Funktion für mehrere Mitgliedschaften ist nicht aktiviert.', 'membership2' ),
		);
	...
	static public function translate_jquery_validator() {
        ...
        required: "<?php _e( 'Dieses Feld ist erforderlich.', 'membership2' ); ?>",
        remote: "<?php _e( 'Bitte korrigiere dieses Feld.', 'membership2' ); ?>",
        email: "<?php _e( 'Bitte gib eine gültige E-Mail-Adresse ein.', 'membership2' ); ?>",
        url: "<?php _e( 'Bitte gib eine gültige URL ein.', 'membership2' ); ?>",
        date: "<?php _e( 'Bitte gib ein korrektes Datum an.', 'membership2' ); ?>",
        dateISO: "<?php _e( 'Bitte gib ein gültiges Datum (ISO) ein.', 'membership2' ); ?>",
        number: "<?php _e( 'Bitte gib eine gültige Nummer ein.', 'membership2' ); ?>",
        digits: "<?php _e( 'Bitte gib nur Ziffern ein.', 'membership2' ); ?>",
        creditcard: "<?php _e( 'Bitte gib eine gültige Kreditkartennummer ein.', 'membership2' ); ?>",
        equalTo: "<?php _e( 'Bitte gib den gleichen Wert erneut ein.', 'membership2' ); ?>",
        maxlength: jQuery.validator.format( "<?php _e( 'Bitte gib nicht mehr als {0} Zeichen ein.', 'membership2' ); ?>" ),
        minlength: jQuery.validator.format( "<?php _e( 'Bitte gib mindestens {0} Zeichen ein.', 'membership2' ); ?>" ),
        rangelength: jQuery.validator.format( "<?php _e( 'Bitte gib einen Wert zwischen {0} und {1} Zeichen ein.', 'membership2' ); ?>" ),
        range: jQuery.validator.format( "<?php _e( 'Bitte gib einen Wert zwischen {0} und {1} ein.', 'membership2' ); ?>" ),
        max: jQuery.validator.format( "<?php _e( 'Bitte gib einen Wert kleiner oder gleich {0} ein.', 'membership2' ); ?>" ),
        min: jQuery.validator.format( "<?php _e( 'Bitte gib einen Wert größer oder gleich {0} ein.', 'membership2' ); ?>" )
        });
		<?php
		$script = ob_get_clean();
	...
	function register_exporter( $exporters ) {
		$exporters['membership2'] = array(
			'exporter_friendly_name' => __( 'Mitgliedschaften Exporteur', 'membership2' ),
	...
	function register_eraser( $erasers ) {
		$erasers['membership2'] = array(
			'eraser_friendly_name' => __( 'Mitgliedschaften Löschtool', 'membership2' ),
	...
	function erase_data( $email_address, $page = 1 ) {
		...
		return array(
			...
			'messages'       => array( __( 'Alle Abonnementdaten gelöscht', 'membership2' ) ),
			'done'           => true,
		);
	}
		...
		$pointers['ms_admin_pointer'] = array(
			...
			'options' => array(
				'content'  => sprintf( '<h3> %s </h3> <p> %s </p>',
					__( 'Erstelle Mitgliedschaften', 'membership2' ),
					__( 'Erstelle hier Mitgliedschaftsstufen mit Paywalls, Passwortschutz und zeitversetztem Inhalt', 'membership2' )
				),
