...
class MS_Controller_Billing extends MS_Controller {
	...
	public function ajax_link_data_transaction() {
		...
		if ( self::validate_required( $fields ) && $this->verify_nonce() ) {
			...
			if ( 'subscriptions' == $type ) {
				...
				$resp[0] 	= __( 'Wähle ein Abonnement', 'membership2' );
				$active 	= array();
				$inactive 	= array();
				foreach ( $member->subscriptions as $subscription ) {
					...
					$line = sprintf(
						__( 'Mitgliedschaft: %s, Grundpreis: %s', 'membership2' ),
						$membership->name,
						$price
					);
				...
				if ( ! count( $active ) && ! count( $inactive ) ) {
					$resp[0] = __( 'Keine Abonnements gefunden', 'membership2' );
				} else {
					if ( count( $active ) ) {
						$resp[__( 'Aktive Abonnements', 'membership2' )] = $active;
					}
					if ( count( $inactive ) ) {
						$resp[__( 'Abgelaufene Abonnements', 'membership2' )] = $inactive;
					}
				}
				...
				$resp[0] 	= __( 'Wähle eine Rechnung aus', 'membership2' );
				$unpaid 	= array();
				$paid 		= array();
				foreach ( $invoices as $invoice ) {
					$line = sprintf(
						__( 'Rechnung: %s von %s (%s)', 'membership2' ),
				...
				if ( ! count( $unpaid ) && ! count( $paid ) ) {
					$resp[0] = __( 'Keine Rechnungen gefunden', 'membership2' );
				} else {
					if ( count( $unpaid ) ) {
						$resp[__( 'Unbezahlte Rechnungen', 'membership2' )] = $unpaid;
					}
					if ( count( $paid ) ) {
						$resp[__( 'Bezahlte Rechnungen', 'membership2' )] = $paid;
					}
				}
			}
		}
	...
	private function save_invoice( $fields ) {
		...
		if ( $this->is_admin_user()
			...
			$invoice = MS_Factory::load( 'MS_Model_Invoice', $invoice_id );
			$this->log( 'Manuelle Rechnungserstellung' );
			if ( ! $invoice->is_valid() ) {
	...
	public function enqueue_scripts() {
		...
		if ( isset( $_GET['action'] ) && 'edit' == $_GET['action'] ) {
			...
			if ( 'logs' == $module || 'matching' == $module ) {
				...
				$data['lang'] = array(
					'link_title' => __( 'Link Transaktion', 'membership2' ),
				);
			}
		}
